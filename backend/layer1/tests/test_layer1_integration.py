...
from backend.layer1 import api as layer1_api
from sqlalchemy import select
from sqlalchemy.orm import Session

from backend.shared.db import get_engine
from backend.main import app
from backend.layer2.models import DiscoveredArticleRecord, DiscoveryRequest
from backend.layer1 import service as layer1
...
def test_google_news_decoder_unavailability_only_fails_that_article(monkeypatch) -> None:
    monkeypatch.setattr(layer1, "gnewsdecoder", None)

    article = layer1._extract_one("https://news.google.com/rss/articles/example")

    assert article.success is False
    assert article.error is not None
    assert "decoding is unavailable" in article.error
...
def test_extract_endpoint_preserves_the_layer1_response_envelope(monkeypatch) -> None:
    ...
    monkeypatch.setattr("backend.layer1.api.extract", fake_extract)
    with TestClient(app) as client:
