...
def test_resolves_exact_entity_matches_beyond_the_candidate_window() -> None:
    names = [f"Bureau Of Example Affairs {index}" for index in range(15)]
    shared = {
        "schema_version": "1.0",
        "sources": [{"source_id": "src-1"}],
        "articles": [],
...
def test_retains_an_ambiguous_entity_as_a_possible_match() -> None:
    ...
    with Session(get_engine()) as session:
        ...
        assert decision is not None
        assert decision.signals_json["candidate_policy_version"] == "exhaustive-v3"
        assert decision.signals_json["candidate_limit"] is None
