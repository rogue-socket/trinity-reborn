...
from backend.shared.artifact_paths import atomic_write_bytes, atomic_write_text
...
def _write_episode(episode: Episode) -> None:
    ...
    atomic_write_text(path, json.dumps(episode.model_dump(mode="json"), indent=2, ensure_ascii=False) + "\n")
...
def load_or_create_voice_profile(topic_id: str, character_id: str) -> VoiceProfile:
    ...
    path.parent.mkdir(parents=True, exist_ok=True)
    atomic_write_text(path, json.dumps(profile.model_dump(), indent=2) + "\n")
    return profile
