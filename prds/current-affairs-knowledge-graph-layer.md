...
### 7.10 Layer 1 ↔ Layer 2 integration contract

This section is the implementation handoff for the Layer 1 and Layer 2 teams. The OpenAPI definition generated by Layer 2 must use the same semantics; it may add only backwards-compatible optional fields without a schema-version change.

#### Ownership boundary

| Concern | Layer 1 owns | Layer 2 owns |
| --- | --- | --- |
| Research collection and article extraction | Yes | No |
| Package-local IDs and self-contained references | Yes | No |
| Evidence excerpts and offsets into supplied article text | Yes | Validates and preserves |
| Source/extraction assessments | May provide | Stores and may use as one signal |
| Canonical IDs and topic registry | No | Yes |
| Entity/event matching and graph merge decisions | No | Yes |
| Duplicate, correction, contradiction, retraction, and progression classification | No | Yes |
| Canonical graph status and query results | No | Yes |

Layer 1 must not wait for, persist, or depend on canonical IDs returned by Layer 2. A later Layer 1 package must remain valid even if Layer 1 has never received an earlier ingestion response.
...
The request body is one package conforming to `schema_version`. It must include:

| Field | Requirement |
| --- | --- |
| `schema_version` | Required. Must equal a Layer 2-supported version. |
| `package_id` | Required. New UUID for each logical delivery; reused only for retrying the same delivery. |
| `topic_key` | Required. Must have been registered by Layer 2 before delivery. |
| `metadata.title` | Required human-readable title for operator visibility. |
| `metadata.generated_at` | Required ISO 8601 timestamp describing when Layer 1 generated the delta. |
| `sources`, `articles`, `evidence`, `entities`, `events`, `claims` | Required arrays. Empty arrays are valid when no object of that type is present. |
| every local reference | Must resolve inside the same package. |

Layer 1 must send UTF-8 JSON. Exact received bytes are retained by Layer 2 for audit; Layer 1 must not mutate a package after it has been assigned a `package_id`.

#### Required object-field rules

| Object | Required fields | Cross-reference rules |
| --- | --- | --- |
| Source | `source_id` | May be referenced by articles. |
| Article | `article_id`, `source_id`, `content` | `source_id` must exist in `sources`; `content` is the received immutable text. |
| Evidence | `evidence_id`, `article_id`, `excerpt`, `start_offset`, `end_offset` | `article_id` must exist; offsets must select the supplied excerpt from article content. |
| Entity | `entity_id`, `type`, `name` | `type` should use the controlled taxonomy or preserve its original type label. |
| Event | `event_id`, `type`, `title` | All participant, location, evidence, and claim references must exist in this package. |
| Claim | `claim_id`, `text` | Every `evidence_id`, event, entity, or attribution reference must exist in this package. |
| Relationship candidate | `relationship_id`, `subject_ref`, `object_ref`, `source_relation_label` | Endpoints and evidence references must exist in this package. |

Layer 1 should supply at least one evidence record for every claim and every relationship candidate. An evidence-free object may be accepted with a warning when allowed by the object type, but it is not eligible for high-confidence canonical promotion.

#### Delivery response

Layer 2 returns:

| HTTP status | Meaning | Layer 1 action |
| --- | --- | --- |
| `201 Created` | New package processed. | Record the report for diagnostics; continue with later independent deltas. |
| `200 OK` | Identical retry already processed. | Treat as successful; do not resend a changed body under the same retry intent. |
| `400 Bad Request` | Invalid envelope or unsupported schema. | Fix the package contract error and send a new delivery. |
| `404 Not Found` | Unknown `topic_key`. | Register/confirm the topic with Layer 2 before retrying. |
| `422 Unprocessable Entity` | Valid package envelope, but no submitted object was eligible for acceptance. | Inspect `object_results`, fix the rejected objects, and send a new delivery. |
| `500` | Layer 2 failed before returning a final report. | Retry the exact same package body and `package_id`. |

The response is an ingestion report. Its canonical IDs and classifications are diagnostic output for Layer 1 and the operator dashboard, not values Layer 1 must send in future packages.

#### Retry and revision rules

1. If Layer 1 does not receive a response, retry the **exact same body** with the same `package_id`.
2. If Layer 1 discovers new or changed material, create a **new `package_id`** and send a new self-contained delta.
3. Never reuse a prior package's local IDs as references in a later package.
4. Never use omission from a later package to signal a deletion, correction, or retraction.
5. Do not attempt to calculate canonical duplicates in Layer 1; send the source-backed candidate and let Layer 2 decide.

#### Contract evolution

- `schema_version` is the compatibility boundary.
- Layer 2 may add optional response fields without changing the schema version.
- New required request fields, changed enum meaning, or changed reference semantics require a new schema version.
- Layer 1 must reject or quarantine packages it cannot build for the negotiated version rather than guessing missing values.
- Layer 2 must return a readable unsupported-version error including the supported versions.

### 7.11 Layer 3 ↔ Layer 2 integration contract

Layer 3 does not call Layer 1 and does not access the Layer 2 database. It calls Layer 2 retrieval APIs exclusively.

#### What Layer 3 receives

Layer 3 receives Layer 2 canonical identifiers and graph knowledge:

- topic, entity, event, claim, and relationship IDs;
- normalized labels and original claim wording where requested;
- event order, temporal precision, and topic-local graph structure;
- statuses such as `active`, `disputed`, `superseded`, and `retracted`;
- epistemic categories such as `reported`, `attributed`, and `inferred`;
- Layer 2 support summaries and confidence dimensions;
- coverage, pagination, and graph-expansion hints.

Layer 3 never receives:
...
- raw source metadata;
- Layer 1 delivery/audit details;
- direct database credentials or graph-database access.

#### Retrieval sequence

The expected Layer 3 sequence is:

1. Call `GET /topics/{topic_id}/context` to obtain a self-sufficient story context.
2. Inspect `coverage`, `disputes`, statuses, and `query_hints`.
3. Call `POST /graph/expand` for bounded detail around canonical graph IDs.
4. Call `GET /claims/{claim_id}/evidence` or `GET /topics/{topic_id}/raw-export` when original wording is needed.
5. Treat status and epistemic category as part of the returned information, not optional metadata.

Layer 3 must not assume that the first returned claim is the only or current perspective when a response contains a dispute, correction, retraction, or truncation indicator.

#### Retrieval compatibility rules

- Layer 3 must include a `topic_id` on every graph-expansion request.
- Layer 3 must respect `next_cursor` and cannot request an unbounded traversal.
- Layer 3 must handle missing optional fields and unknown future status values safely.
- Layer 3 must treat `as_of` as a historical knowledge view, not a claim that past information remains current.
- Raw exports contain original story-relevant wording but remain redacted of Layer 1 source and transport metadata.
